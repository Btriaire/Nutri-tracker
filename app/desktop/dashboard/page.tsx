...
import DashboardClientDesktop from "@/app/components/DashboardClientDesktop";

export default async function DesktopDashboardPage() {
  ...
  try {
    ...
    for (const doc of healthSnap.docs) {
      const d = doc.data() as { date?: string; bloodPressure?: { systolic: number }[] };
  ...
  const manualActivities = (manualActivitiesSnap?.docs ?? []).map(d => {
    ...
    const startMs = (a.loggedAt as unknown as { _seconds?: number })?._seconds
      ...
      : Date.now();
    return { id: a.id, name: a.name, activityType: a.activityType, durationMin: a.durationMin, startMs, calories: a.caloriesBurned ?? null };
  });
  const allSessions = [...gfitSessions, ...manualActivities].sort((a, b) => a.startMs - b.startMs);
  ...
  const totalBurned  = (gfitBurned ?? 0) + manualBurned > 0 ? (gfitBurned ?? 0) + manualBurned : null;

  return (
    <DashboardClientDesktop
      date={today}
