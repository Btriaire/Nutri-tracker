...
import LogClientDesktop from "@/app/components/LogClientDesktop";
...
export default async function DesktopLogDatePage({ params }: Props) {
  const { date } = await params;
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) redirect("/desktop/log");
  ...
  return <LogClientDesktop date={date} initialLog={dayLog} goals={goals} lang={lang} />;
}
