...
import { IconX, IconSparkles, IconCheck, IconPlus, IconMinus, IconMicrophone, IconPlayerStopFilled } from "@tabler/icons-react";
...
const MEAL_META: Record<MealType, { label: string; emoji: string; color: string }> = {
  breakfast: { label: "Petit-déj",  emoji: "🍳", color: "#fbbf24" },
  lunch:     { label: "Déjeuner",   emoji: "🥗", color: "#34d399" },
  dinner:    { label: "Dîner",      emoji: "🍽️", color: "#a78bfa" },
  snacks:    { label: "Collation",  emoji: "🍎", color: "#f97316" },
};
const MEAL_ORDER: MealType[] = ["breakfast", "lunch", "dinner", "snacks"];

const ACCENT = "#34d399"; // Nutri-IA green

function defaultMealFromTime(): MealType {
...
function MacroBar({ proteinG, carbsG, fatG, height = 5 }: { proteinG: number; carbsG: number; fatG: number; height?: number }) {
  ...
  return (
    <svg viewBox="0 0 200 6" width="100%" height={height} style={{ display: "block" }}>
      ...
      <rect x={pP}      y={0} width={pC} height={6}        fill="#fbbf24" />
      <rect x={pP + pC} y={0} width={pF} height={6}        fill="#a78bfa" />
...
export default function VoiceMealModal({ date, onAdded, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    recog.onerror = (e) => {
      if (e.error !== "no-speech" && e.error !== "aborted") {
        setError("Micro indisponible — vous pouvez taper votre repas ci-dessous");
      }
      setListening(false);
    };
  ...
  const analyze = async () => {
    ...
    if (!text) { setError("Dites ou écrivez d'abord votre repas"); return; }
    stopListening();
    setPhase("analyzing");
    setError(null);
    try {
      const res = await fetch("/api/food/voice", {
        ...
        body:    JSON.stringify({ text, defaultMeal: defaultMealFromTime() }),
      });
      if (!res.ok) throw new Error("api");
      const data = await res.json() as {
        results: { result: FoodSearchResult; meal: MealType; per100g: Per100g; grams: number }[];
      };
      const detected = data.results ?? [];
      if (detected.length === 0) {
        setError("Aucun aliment reconnu — reformulez (ex: « 150g de poulet et du riz »)");
        setPhase("idle");
        return;
      }
      setItems(detected.map((d) => ({
        result:   d.result,
        per100g:  d.per100g,
        grams:    d.grams,
        meal:     d.meal,
  ...
  const adjustGrams = (idx: number, delta: number) =>
    setItems((prev) => prev.map((it, i) => i !== idx ? it : { ...it, grams: Math.max(5, it.grams + delta) }));
  const setGrams = (idx: number, g: number) =>
    setItems((prev) => prev.map((it, i) => i !== idx ? it : { ...it, grams: Math.max(5, Math.min(2000, g || 5)) }));
  const toggleSelect = (idx: number) =>
    setItems((prev) => prev.map((it, i) => i !== idx ? it : { ...it, selected: !it.selected }));
  const cycleMeal = (idx: number) =>
    setItems((prev) => prev.map((it, i) => {
      if (i !== idx) return it;
      const next = MEAL_ORDER[(MEAL_ORDER.indexOf(it.meal) + 1) % MEAL_ORDER.length];
      return { ...it, meal: next };
    }));

  const selected = items.filter((it) => it.selected);
  const totals   = selected.reduce((acc, it) => {
    const n = scale(it);
          ...
          {phase === "analyzing" && (
            <div className="flex flex-col items-center py-8 gap-5">
              ...
              <div className="text-center">
                ...
                <p className="text-[12px] mt-1 px-6" style={{ color: "var(--text-muted)" }}>Nutri-IA identifie vos aliments</p>
              </div>
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 16 }}
              ...
              <button onClick={handleAdd} disabled={phase === "saving"}
                ...
                {phase === "saving" ? (
                  ...
                  <><IconCheck size={16} stroke={2.5} /> Ajouter {selected.length} aliment{selected.length > 1 ? "s" : ""} · {totals.calories} kcal</>
                )}
              </button>
            </motion.div>
          )}
