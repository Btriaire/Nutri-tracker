...
import {
  IconArrowDown, IconArrowUp, IconMinus, IconScale, IconChartBar,
  IconCalendar, IconFlame, IconMoon, IconLoader2,
...
export default function ProgressClient({ goals, currentWeightKg, targetWeightKg, targetDate: initialTargetDate, age, plan: initialPlan, trackedNutrients }: Props) {
  ...
  // Load alcohol data for the last 7 days — shown in Progress when alcoholTracking is on
  useEffect(() => {
    if (!goals.alcoholTracking) return;
    const to   = format(new Date(), "yyyy-MM-dd");
    const from = format(subDays(new Date(), 6), "yyyy-MM-dd");
    fetch(`/api/progress?from=${from}&to=${to}`)
      .then(r => r.json())
      .then((d: { points?: DayTrendPoint[] }) => {
        const pts = (d.points ?? []).map(p => ({
          date:        p.date,
          alcoolUnits: p.alcoolUnits ?? 0,
          label:       format(parseISO(p.date), "EEE", { locale: fr }).slice(0, 3),
        }));
        setAlcoolWeekPts(pts);
      })
      .catch(() => {});
  ...
  return (
    <div className="relative min-h-screen" style={{ paddingBottom: "80px" }}>
      ...
      <div className="relative z-10 max-w-md mx-auto px-4 py-6 md:ml-[220px] md:max-w-2xl">
        ...
        {range === "1j" && (
          <TodayView todayPoint={todayPoint} loading={loading} goals={goals} targetWeightKg={targetWeightKg} />
        )}
        ...
        {range !== "1j" && (
          <>
              ...
              <motion.div id="poids" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {weightDelta !== null && (
                      <span className="flex items-center gap-1 text-[12px] font-medium"
                        style={{ color: weightDelta < -0.1 ? "var(--ok)" : weightDelta > 0.1 ? "var(--danger)" : "var(--text-muted)" }}>
                ...
                <div className="flex gap-2 mb-3">
                  {[
                    ...
                    { label: "Objectif", value: targetWeightKg   ? `${targetWeightKg.toFixed(1)} kg`  : "—", color: "var(--ok)" },
                    { label: "Écart",
                      value: (effectiveCurrentWeight && targetWeightKg) ? `${Math.abs(effectiveCurrentWeight - targetWeightKg).toFixed(1)} kg` : "—",
                      color: (effectiveCurrentWeight && targetWeightKg && effectiveCurrentWeight > targetWeightKg) ? "var(--danger)" : "var(--ok)" },
                    { label: "Date cible",
                      ...
                      <ComposedChart data={showAdequacyColoring ? weightChartDataColored : weightChartData} margin={{ top: 8, right: 16, left: -10, bottom: 0 }}>
                        <defs>
                          ...
                          <linearGradient id="bandGrad" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="5%"  stopColor="var(--ok)" stopOpacity={0.08} />
                        ...
                        {targetWeightKg && (
                          <ReferenceLine yAxisId="w" y={targetWeightKg}
                            ...
                            label={{ value: `Objectif ${targetWeightKg}kg`, fontSize: 11, fill: "var(--ok)", position: "insideTopRight" }} />
                        )}
                        {/* Today line */}
                        ...
                        <Line yAxisId="w" type="monotone" dataKey="projected" name="projected"
                          stroke="var(--ok)" strokeWidth={1.25} strokeDasharray="5 3"
                          ...
                          dot={(props: any) => {
                            ...
                            return <circle key={`p-${payload.date}`} cx={cx} cy={cy} r={1.8} fill="var(--ok)" stroke="var(--bg)" strokeWidth={1} />;
                          }}
                          activeDot={{ r: 3, fill: "var(--ok)" }} connectNulls />
            ...
            {caloriePoints.length > 0 && (
              <NutrientsEvolution caloriePoints={caloriePoints} goals={goals} trackedNutrients={trackedNutrients} range={range} />
            )}
            ...
            {/* ── Alcohol weekly tracker ── */}
            {goals.alcoholTracking && (
              <AlcoolWeekWidget
                pts={alcoolWeekPts}
                weeklyGoal={goals.weeklyAlcoolUnitsGoal ?? 14}
              />
            )}
